...
export default function Curso() {
  ...
  useEffect(() => {
    ...
    const totalAulas = 6;
    const aulasGeradas = Array.from({ length: totalAulas }, (_, i) => ({
      id: i + 1,
      nome: `Aula ${i + 1}`,
      duracao: `${20 + i * 5} min`,
      concluida: i < Math.floor((progresso / 100) * totalAulas),
    }));
    ...
    <div className="min-h-screen  p-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-6">
          <button
            onClick={() => navigate(-1)}
            className="flex items-center gap-2 px-4 py-2 text-white rounded-lg bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700"
          >
            <ArrowLeft className="w-4 text-white h-4" />
            Voltar
          </button>

          <div className="flex-1">
            <h1 className="text-xl font-bold ">{curso.nome}</h1>
            <p className="text-sm ">{curso.carreira}</p>
          </div>
        </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm">
              <div className="flex items-start gap-4 mb-4">
                <div className="p-3 bg-gradient-to-br from-cyan-500 to-blue-500 rounded-xl shadow-md">
                  <span className="text-2xl">{curso.emoji || "📘"}</span>
                </div>
                <div className="flex-1">
                  <h2 className="font-bold text-white text-lg mb-2">{curso.nome}</h2>
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">{curso.descricao}</p>
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="px-2.5 py-1 bg-cyan-100 dark:bg-cyan-900/30 text-cyan-700 dark:text-cyan-300 text-xs font-medium rounded-full">
                      ⏱️ {curso.horas}h
                    </span>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium text-gray-900 dark:text-white">Possíveis profissões</p>
                    <p className="text-xs text-gray-600 dark:text-gray-400">
                      {curso.profissoesPossiveis?.join(" • ")}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
              ...
              <div className="text-center mb-4">
                ...
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3">
                  <div className="h-full bg-cyan-600 rounded-full transition-all" style={{ width: `${progresso}%` }} />
                </div>
              </div>

              <div className="grid grid-cols-2 gap-2 text-center">
                <div className="p-2 bg-gray-50 dark:bg-gray-700 rounded-lg">
                  <div className="font-bold text-gray-900 dark:text-white">{aulas.filter(a => a.concluida).length}</div>
                  <div className="text-xs text-gray-600 dark:text-gray-400">Concluídas</div>
                </div>
                <div className="p-2 bg-gray-50 dark:bg-gray-700 rounded-lg">
                  <div className="font-bold text-gray-900 dark:text-white">{aulas.length - aulas.filter(a => a.concluida).length}</div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
                ...
                {aulas.map((aula) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`p-2 rounded-lg ${aula.concluida ? "bg-green-100 dark:bg-green-900/30" : "bg-cyan-100 dark:bg-cyan-900/30"}`}>
                        {aula.concluida ? <CheckCircle className="w-5 h-5 text-green-500" /> : <PlayCircle className="w-5 h-5 text-cyan-500" />}
                      </div>
                      <div>
                        <h3 className={`font-medium ${aula.concluida ? "text-gray-500 line-through" : "text-gray-900 dark:text-white"}`}>{aula.nome}</h3>
              ...
              {progresso === 100 && (
                <div className="mt-4 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg text-center">
                  ...
                  <p className="text-green-800 dark:text-green-200 font-medium">Curso concluído com sucesso! 🎉</p>
                </div>
              )}
            </div>
          </div>
